...
**최종 렌더**(1080p/30fps) 가 끝나면 **검증** 탭에 21개 항목이 PASS/FAIL 로 나옵니다: 해상도·fps·코덱, 오디오/비디오 길이 일치, 세그먼트 순서, 내레이션 잘림 없음, 발화 비율(55~98%), 자막 겹침/길이/누락, 검은 프레임·정지 화면·선명도, 라우드니스(−16 LUFS)·클리핑·긴 무음. 하나라도 FAIL 이면 원인을 고치고 다시 렌더합니다(예: 발화 비율 99% → 세그먼트 여백을 1초로).
