...
def build_f_prime_graph() -> VGroup:
    ...
    dots = VGroup(
        Dot(axes.coords_to_point(-1, 0), color=YELLOW, radius=0.06),
        Dot(axes.coords_to_point(1, 0), color=YELLOW, radius=0.06),
        Dot(axes.coords_to_point(0, 6), color=RED, radius=0.06),
    )
    lbl = MathTex(r"y=f'(x)", font_size=18, color=BLUE).next_to(axes, UP, buff=0.04)
    return VGroup(axes, curve, dots, lbl)
...
def build_f_cubic_graph(k: float = 10.0) -> VGroup:
    ...
    lbl = MathTex(r"y=f(x)", font_size=18, color=TEAL).next_to(axes, UP, buff=0.04)
    return VGroup(axes, curve, hline, lbl)
...
def build_g_graph(k: float = 10.0, t_line: float = 13.0) -> VGroup:
    ...
    t_lbl = MathTex(r"y=13", font_size=18, color=RED).next_to(axes.coords_to_point(1.2, t_line), UP, buff=0.04)
    lbl = MathTex(r"y=g(x)", font_size=18, color=ORANGE).next_to(axes, UP, buff=0.04)
    return VGroup(axes, left, right1, right2, t_h, t_lbl, lbl)
