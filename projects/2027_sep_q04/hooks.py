...
def piecewise_gap(scene, left_expr: str, right_expr: str, split, param: str, values: list, id: str = "pw",
                  ...
                  right_label: str | None = None, left_label_at=None, right_label_at=None,
                  stop_captions: list | None = None, pause: float = 0.6,
    ...
    value = DecimalNumber(values[0], num_decimal_places=0, color=scene.theme.highlight).scale(0.8)
    ...
    labels = VGroup()
    if left_label:
        xl = scene.eval(left_label_at) if left_label_at is not None else x_s - 1.0
        lm = scene.mtex(left_label, scale=0.7, plain=True).set_color(lcol)
        lm.next_to(scene.c2p(xl, fl()(xl)), UP, buff=0.32)
        labels.add(lm)
    if right_label:
        xrr = scene.eval(right_label_at) if right_label_at is not None else x_s + 1.0
        rm = scene.mtex(right_label, scale=0.7, plain=True).set_color(rcol)
        rm.next_to(scene.c2p(xrr, fr()(xrr)), DR, buff=0.15)
        labels.add(rm)
    for m in labels:
        m.set_z_index(9)
    ...
    scene._pw = {"tracker": tracker, "dyn": (left, right, ends, gap, value), "id": id,
                 "mobs": {id: VGroup(left, right), f"{id}_ends": ends, f"{id}_readout": box, f"{id}_vline": vline,
                          f"{id}_labels": labels}}
    if len(values) > 1:
        piecewise_sweep(scene, values=values[1:], run_time=run_time, pause=pause,
                        stop_captions=(stop_captions or [None])[1:])
    else:
        for k, m in scene._pw["mobs"].items():
            scene.register(k, m)


def piecewise_sweep(scene, values: list, run_time: float = 4.0, pause: float = 0.6, stop_captions: list | None = None,
                    caption: str | None = None, **_):
    """`piecewise_gap` 으로 그린 그래프의 a 를 values 순서로 움직인다 (내레이션 `at` 에 맞춰 따로 호출)."""
    st = scene._pw
    tracker = st["tracker"]
    values = [scene.eval(v) for v in values]
    seq = [tracker.get_value()] + values
    total = sum(abs(b - a) for a, b in zip(seq, seq[1:])) or 1.0
    for i, (a, b) in enumerate(zip(seq, seq[1:])):
        ...
        text = None
        if stop_captions and i < len(stop_captions):
            text = stop_captions[i]
        if i == len(values) - 1 and caption:
            text = caption
        if text:
            caption_action(scene, text=text, run_time=0.4)
        if pause > 0:
            scene.wait(pause)
    for m in st["dyn"]:
        m.clear_updaters()
    for k, m in st["mobs"].items():
        scene.register(k, m)
